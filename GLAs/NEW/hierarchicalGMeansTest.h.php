...
function G_Means(array $t_args, array $inputs, array $outputs)
{
    ...
    $setterCode = eval_implode('"item[$counter] = $name1;"', PHP_EOL, $inputs);
    $parameterCode = const_typed_ref_args($inputs);
?>
...
class <?=$className?>Cluster {
 ...
 private:
  ...
  // This acts as both the sum of the scalar projections and later as the mean
  // after it is divided by the count.
  double mean_sum;
  ...
  // TODO: Change these definitions to be sum of cubes, etc or merge.

  // The second moment about the mean, i.e. sum of (x - mean)^2. Used for
  // calculation standard deviation for both skewness and kurtosis.
  long double second_moment;

  // The third moment about the mean, i.e. sum of (x - mean)^3. Used for
  // calculation of skewness.
  long double third_moment;

  // The fourth moment about the mean, i.e. sum of (x - mean)^4. Used for
  // calculation of kurtosis.
  long double fourth_moment;
 ...
 public:
  ...
  void AddItem(vec point) {
    switch (state) {
      ...
      case State::kMeansChildren:  // Filter the data for k-means.
        ...
        break;
      case State::projection:  // Compute the scalar mean.
        count++;
        mean_sum += dot(point, v) / v_norm;
        break;
      case State::jarqueBera:  // Compute kurtosis and skewness.
        projection     = dot(point, v) / v_norm - mean_sum;
        dummy_product  = projection * projection;
        second_moment += dummy_product;
        dummy_product *= projection;
        third_moment  += dummy_product;
        fourth_moment += dummy_product * projection;
        count ++;
        break;
  ...
  void AddState(<?=$className?>Cluster & other) {
    switch (state) {
      ...
      case State::kMeansChildren:  // Sum the 2 K-means children.
        ...
        break;
      case State::projection:  // Compute the overall sum.
        // TODO: This could probably be grabbed from the kMeans children
        // Projections are linear additive.
        mean_sum += other.mean_sum;
        count += other.count;
        break;
      case State::jarqueBera:  // Compute the sum of moments and counts.
        second_moment += other.second_moment;
        third_moment  += other.third_moment;
        fourth_moment += other.fourth_moment;
        count += other.count;
        break;
  ...
  void DoIteration(<?=$className?>Node &node) {
    ...
    vec max_eigenvector;
    vec previous_center1;
    vec previous_center2;
    ...
    switch (state) {
      ...
      case State::gMeans:  // Find eigenvalues and find PCA
        ...
        max_eigenvalue = eigenvalues.max(index);
        max_eigenvector = eigenvectors.col(index);
      ...
      case State::kMeansChildren:  // Check for convergence and advance k-means.
        previous_center1 = child1->center;
        previous_center2 = child2->center;
        ...
        if (   HasConverged(previous_center1, node.child1->center)
          ...
          node.v = v;
          node.state = State::projection;
          node.child1->state = State::gMeans;
          node.child2->state = State::gMeans;
        }
        break;
      case State::projection:  // Compute the mean.
        mean_sum /= count;
        node.mean_sum = mean_sum;
      ...
      case State::jarqueBera:  // Compute kurtosis and skewness; run test.
        ...
        kurtosis = count * fourth_moment / pow(second_moment, 2);
        skewness = sqrt(count)
                             * third_moment / pow(second_moment, 1.5);
  ...
  void ConstructTree(<?=$className?>Node &node) {
    node.center = center;
    node.state = state;
    node.mean_sum = mean_sum;
    node.v = v;
    switch (state) {
      ...
      case State::activeParent:
      case State::inactiveParent:
        child1->ConstructTree(*node.child1);
        child2->ConstructTree(*node.child2);
        break;
    }
  }
};
 ...
 public:
  <?=$className?>(const <?=$constantState?> & state)
      : constant_state(state),
        root(state.root),
        item(<?=$dimension?>) {
   //constant_state.root.GetOutput(0);
  }

  void AddItem(<?=$parameterCode?>) {
    <?=$setterCode, PHP_EOL?>
