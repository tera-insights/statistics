...
class <?=$className?>Node {
 ...
 public:
  ...
  Json::Value GetOutput(int depth) const {
...
<?  }  ?>
    if (child1 != nullptr) data["offspring1"] = child1->GetOutput(depth + 1);
    if (child2 != nullptr) data["offspring2"] = child2->GetOutput(depth + 1);
    data["depth"] = depth;
    data["count"] = count;
    return data;
  }
...
class <?=$className?>Cluster {
 ...
 private:
  ...
  // These represents the sums of the projections raised to some power.
  ...
  void AddItem(vec point) {
    switch (state) {
      ...
      case State::jarqueBera:  // Compute kurtosis and skewness.
        ...
        sum_second += dummy_product;
        dummy_product *= projection;
        sum_third  += dummy_product;
        sum_fourth += dummy_product * projection;
        break;
