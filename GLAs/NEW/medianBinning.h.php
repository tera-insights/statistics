...
class <?=$className?>ConstantState{
 private:
  ...
  // Vectors used during stage 2a. It should be noted that elements of max_2a
  // hold the maximum value of the lower relevant bucket and the corresponding
  // elements of min_2a hold the minimum value of the higher bucket. Hence,
  // the values of max_2a are actually less than those of min_2a, which might be
...
<?  } ?>
      ...
      // It should be noted that the search for lower_max and upper_min will
      // only happen once per attribute even if the attributes exist in stage_2a
      // for multiple iterations because stage_2a(n) is incremented to have
      // value 2 in ShouldIterate after the first iteration in which the nth
      // attribute is in stage 2a.
<?  foreach ($codeArray as $name => $counter) { ?>
      if (   constant_state.stage_2a(<?=$counter?>) == 1
  ...
  bool ShouldIterate(<?=$constantState?> & modible_state) {
    if (constant_state.iteration == 0) {
      ...
      if (all(modible_state.stage_2a)) {
        for (int counter = 0; counter < <?=$dimension?>; counter++)
          if (modible_state.stage_2a(counter) == 1) {
            modible_state.max_2a(counter) = lower_max(counter);
            modible_state.min_2a(counter) = upper_min(counter);
          }
        medians = (modible_state.max_2a + modible_state.min_2a) / 2;
        modible_state.final_iteration = true;
        modible_state.iteration++;
        cout << "lower max" << endl << lower_max << endl;
        cout << "upper_min" << endl << upper_min << endl;
        cout << "stage 2a" << endl << modible_state.stage_2a << endl;
        return false;
      }
      ...
      for (int counter = 0; counter < <?=$dimension?>; counter++) {
        if (modible_state.stage_2a(counter) == 1) {
          ...
          modible_state.max_2a(counter) = lower_max(counter);
          modible_state.min_2a(counter) = upper_min(counter);
          modible_state.stage_2a(counter) = 2;
        } else {
