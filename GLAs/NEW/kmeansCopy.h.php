...
class <?=$className?>ConstantState{
 private:
  ...
  mat::fixed<<?=$dimension?>, <?=$numberClusters?>> centers;
...
function K_Means(array $t_args, array $inputs, array $outputs)
{
    ...
    $pMinkowski = $t_args['p'];
    grokit_assert($pMinkowski != 0, "The parameter p must be a positive real.");
...
class <?=$className?>Cluster {
 ...
 public:
  ...
  <?=$className?>Cluster(const vec::fixed<<?=$dimension?>> & _center)
      : count(0),
        center(_center),
        sum() {
    sum.zeros();
  ...
  void AddItem(vec::fixed<<?=$dimension?>> & point) {
    count ++;
    sum += point;
  }
...
class <?=$className?> {
 private:
  ...
  mat::fixed<<?=$dimension?>, <?=$numberClusters?>> shifted_centers;
...
<?  if($startsNeeded) { ?>
    if (constant_state.iteration)
      for (int counter; counter < <?=$numberClusters?>; counter ++)
        clusters.push_back(
            <?=$className?>Cluster(constant_state.centers.col(counter))
        );
<?  } else { ?>
    for (int counter; counter < <?=$numberClusters?>; counter ++)
      clusters.push_back(
...
<?      if ($kMeansPP) { ?>
      ...
      modible_state.centers.col(0) = sample[first_index];
      ...
      for (num_clusters; num_clusters < <?=$numberClusters?>; num_clusters ++) {
        ...
        modible_state.centers.col(num_clusters) = sample[s_counter];
