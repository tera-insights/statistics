...
function Reservoir_Sampling(array $t_args, array $inputs, array $outputs)
{
    ...
    $thresholdCoefficient = $t_args['threshold.coefficient'];
...
class <?=$className?> {
 private:
  ...
  // A constant value generated by the template arguments of the PHP file.
  const double kSampleSize;
  ...
  // subject to change. A constant value generated by the template arguments of
  // the PHP file.
  const double kThresholdCoefficient;

  // The value at which we should switch algorithms to decide skip amount. A
  // constant value that is the product of two template arguments, sample size
  // and threshold value.
  const double kThreshold;

  // An STL vector containing the tuples of data that represent the sample. Due
  // to the small size of the sample, this can be directly referenced in C++.
  vector<TUPLE> sample;
 ...
 public:
  ...
  <?=$className?>()
      : kSampleSize(<?=$sampleSize?>),
        kThresholdCoefficient(<?=$thresholdCoefficient?>),
        kThreshold(kThresholdCoefficient * kSampleSize) ,
        sample(kSampleSize),
        count(0),
        return_counter(0),
        P(-1),
...
<?  } ?>
  ...
  bool GetNextResult(<?=typed_ref_args($inputs)?>) {
    if(return_counter >= kSampleSize)
      return false;
<?  foreach ($codeArray as $name => $counter) { ?>
...
<?  } ?>
  ...
  // A helper function not part of datapath. It is called by GLAs who used
  // this GLA in the body of their code.
...
<?php
    return array(
        'kind' => 'GLA',
        'name' => $className,
        'system_headers' => array('math.h', 'armadillo', 'random', 'vector',
                                  'stdexcept'),
        'input' => $inputs,
        'output' => $inputs,
        'result_type' => 'multi',
